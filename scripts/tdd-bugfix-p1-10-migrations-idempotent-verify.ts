...
// ── 场景 1：全新库 ──
{
  ...
  check('① 全新库迁移不抛错且版本到 13（V13 落地后同步，5db45f7 先例）', threw === '' && versionOf(d) === 13, threw || `version=${versionOf(d)}`);
...
// ── 场景 2：半应用态（ALTER 已执行、版本号 0）——修复主场景 ──
{
  ...
  check('④ 半应用态迁移后版本写到 13 且列齐全', versionOf(d) === 13 && cols(d, 'sessions').includes('provider_override'));
  d.close();
}
