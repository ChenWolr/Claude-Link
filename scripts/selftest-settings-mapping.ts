...
console.log('\n=== 40) UI 简化（r9 定版）：连接页=供应商库 + 会话内不调字号 ===');
{
  ...
  // C-8 同步（2026-09-18）：死样式 .autodetect-bar 三件删除——家具块同宽列断言改钉 .storage-info {
  // （同选择器组第二成员，意图不变：横幅/toast/存储/操作条对齐 --col-w 居中）。
  check('家具块（横幅/存储/操作条）对齐同宽列（--col-w 居中）', cp.includes('.storage-info {') && /banner,\s*\n\s*\.toast,/.test(cp) && cp.includes('max-width: 100%'));
...
console.log('\n=== 46) Claude 计划任务状态（TodoWrite / Task 工具）：类型/repo/IPC/store/UI 契约 ===');
{
  ...
  check('migrations.ts 版本 ≥ 12（V12 会话回合计量列；V11 权限清洗一次性化；原 OPT-8 messages.parent_task_id 索引）', /CURRENT_SCHEMA_VERSION = 1[2-9]/.test(migrations));
...
console.log('\n=== 48) 思考强度接线：持久化层 + 注入层 + IPC 通道契约 ===');
{
  ...
  // 持久化层（Task 4）
  check('migrations.ts schema 版本 ≥ 12（V12 会话回合计量列；V11 权限清洗一次性化；原 OPT-8 messages.parent_task_id 索引）', /CURRENT_SCHEMA_VERSION = 1[2-9]/.test(migrations));
