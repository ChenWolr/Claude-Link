...
//   %USERPROFILE%\.claude\settings.json   → env.* / apiKeyHelper（复用 parseClaudeSettings；apiKeyHelper 仅解析留存，当前无消费方）
...
// apiKeyHelper 仅解析留存、当前无消费方（渲染层 0 处引用，applyExtractedSettings 不回填），
// Claude Link 不执行外部脚本。
