...
- **真实 Electron 窗口核验（Task 9 Step 3，经 CDP 驱动真实运行窗口）**：app 启动含 Task 8 改动无崩溃；`window.claudeLink` preload 桥真实在位且 `getCommandDiagnostics` 已上线；命令菜单来源徽章真实渲染（`/init`→「Claude Code 内置」、user-skill→「用户 Skill」）+ provenance 行「7 个命令来源未知 · 3 个隐藏命令」；`/context` `/usage` 本地命令在真实窗口内执行并渲染结果；`/init` 命令在真实窗口内正确派发（模型接收并响应）。**`/init` 落盘未达成：模型网关的工具调用层（Read/Write）当前降级**——模型明确返回「当前上游工具权限仍未恢复，无法读取代码库或创建 CLAUDE.md」，随后 app **正确触发 `init_write_skipped` 横幅（Task 5 逻辑：不伪造成功）**。尝试过 sonnet(`gpt-5.6-sol[1m]`)/opus(`glm-5.2[1m]`)/`glm-5.2`(no-[1m]) 三种模型均受工具层降级影响；`/init` 代码路径（工具可用时）已由 native E2E init-matrix（10 场景 + 真实 CLAUDE.md 创建，~1h 前网关工具层健康时通过）覆盖。网关工具层恢复后可由用户在真实窗口补 `/init`/`/compact` 落盘核验；`/clear` `/config` 本地但具破坏性/侵入性，未在用户真实会话上执行。
